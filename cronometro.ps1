# & $sd\contador10m.ps1 -Minutos 1 -Espera 15
...
do {
    ...
    for ($min = 0; $min -lt $totalMinutos; $min++) {
        for ($sec = 0; $sec -lt 60; $sec++) {
            ...
            Write-Host ("`rTempo: {0}`tLoops: $loopCount  " -f $tempo) -NoNewline
        ...
        switch ($Espera) {
            {$true} { 
                ...
                for ($i = $Espera; $i -ge 0; $i--) {
                    ...
                    Write-Host ("`rPróxima: {0} `tLoops: $loopCount  " -f $ProximaOnda) -NoNewline -ForegroundColor Red
                    Start-Sleep -Seconds 1
                } 
            }
